...
void Lizard::sago2MonkeyGrassIsSafe()
{
    ...
    while (true) {    //rm
      m_lock.lock();
      if ((!UNIDIRECTIONAL && (sago2grass_val + grass2sago_val) > MAX_LIZARD_CROSSING) || 
          (UNIDIRECTIONAL && sago2grass_val > 0 && grass2sago_val == MAX_LIZARD_CROSSING)) {    //or
        sago2grass_val--;
        m_lock.unlock();
        break;
      } else {
          ...
          return (!UNIDIRECTIONAL && (sago2grass_val + grass2sago_val) > MAX_LIZARD_CROSSING) || 
            (UNIDIRECTIONAL && sago2grass_val > 0 && grass2sago_val == MAX_LIZARD_CROSSING);    //or
        });
      }
    }
...
void Lizard::monkeyGrass2SagoIsSafe()
{
    ...
    while (true) {    //rm
      m_lock.lock();
      if ((!UNIDIRECTIONAL && (grass2sago_val + sago2grass_val) > MAX_LIZARD_CROSSING) || 
          (UNIDIRECTIONAL && grass2sago_val > 0 && sago2grass_val == MAX_LIZARD_CROSSING)) {    //or
        grass2sago_val--;
        m_lock.unlock();
        break;
      } else {
      	...
      	unique_lock<mutex> wait_lock(m_lock);
		    cv_lock.wait(wait_lock, [] {
          return (!UNIDIRECTIONAL && (grass2sago_val + sago2grass_val) > MAX_LIZARD_CROSSING) || 
            (UNIDIRECTIONAL && grass2sago_val > 0 && sago2grass_val == MAX_LIZARD_CROSSING);    //or
        });
      }
    }
...
int main(int argc, char **argv)
{
     ...
     */
    sago2grass_val = MAX_LIZARD_CROSSING;    //or
    grass2sago_val = MAX_LIZARD_CROSSING;
